...
namespace Консоль
{
    class Program
    {
        ...
        //VectorNetFunc func = МатКлассы.FuncMethods.ODU.ODUsearch(f, new Vectors(0,0), 0, 1, 0.01, МатКлассы.FuncMethods.ODU.Method.E1, 0.001, false);
        ...
        //(-1.8).ToPeriod(2).Show();
        //МатКлассы.SpecialFunctions.MyBessel(1, 1450.7).Show();
        ...
        //Func<Complex, CSqMatrix> f = (Complex z) => new CSqMatrix(new Complex[,] { {Complex.Sin(z)/z,Complex.Ch(z)},{0,Complex.Sh(z) } });
        //SumsAndLimits.LimitCircle(f, 0).Show();
        //SumsAndLimits.LimitRandom(f, 0).Show();

        //"0.257755E+00".ToDouble().Show();


        //Matrix<System.Numerics.Complex> matrix = MathNet.Numerics.LinearAlgebra.Complex.DenseMatrix.OfArray(new System.Numerics.Complex[,] { {new System.Numerics.Complex(1,3),2,3 },{2,5,8 },{34,5,2 } });
        //(matrix.Inverse() * matrix).AsArray();

        //var c = FuncMethods.UltraVsNormal(x => x.Sqr()+Math.Sin(3*x), FuncMethods.Monoms, SequenceFuncKind.Other,1,80);
