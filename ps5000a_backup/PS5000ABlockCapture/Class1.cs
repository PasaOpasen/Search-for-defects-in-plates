        ...
        public void SaveOut_old(string filename)
        {
            try
            {
                ...
                using (StreamWriter sw = new StreamWriter(OutFile, false, System.Text.Encoding.Default))
                {
                    ...
                    {
                        Complex w = dw * i + w_0;
                        line = (w.Real /2/Math.PI).ToString() + " " + F[i].Real.ToString() + " " + F[i].Imaginary.ToString();
                        sw.WriteLine(line);
                    }
            ...
            try
            {
                using (StreamWriter sw = new StreamWriter(filename))
                {
                    ...
                    for (int i = 0; i < count_w; i++)
                    {
                        double w = dw *i + w_0;
                        //Debug.WriteLine($"{w} {F[i]} {F[i].Real} {F[i].Imaginary}");
                        sw.WriteLine($"{w / 2 / Math.PI*1000.0} {F[i].Real } {-F[i].Imaginary}");
                    }
            ...
            try
            {
                ...
                using (StreamWriter sw = new StreamWriter(OutFile, false, System.Text.Encoding.Default))
                {
                    ...
                    for (int i = 0; i < count_w; i++)
                    {
                        ...
                        line = (w.Real / 2 / Math.PI * 1000.0).ToString() + " " + F[i].Magnitude.ToString();
                        sw.WriteLine(line);
                    }
