...
namespace PS5000A
{
    public partial class PS5000ABlockForm : Form
    {
        ...
        public PS5000ABlockForm(double w0_, double w1_, int l_)
        {
            ...
            folderbase = Path.Combine(Environment.CurrentDirectory, p);
            textBox12.Text = folderbase;
                SetForlders();
            SetFiles();
        ...
        private void SetFiles()
        {
            ...
            //filenames[0] =Path.Combine(folderbase, $"f(w) from ({textBox1.Text} , {textBox2.Text}).txt");
            //filenames[1] = Path.Combine(folderbase, $"f(w) from ({textBox4.Text} , {textBox3.Text}).txt");
            //filenames[2] = Path.Combine(folderbase, $"f(w) from ({textBox6.Text} , {textBox5.Text}).txt");
            //filenames[3] = Path.Combine(folderbase, $"f(w) from ({textBox8.Text} , {textBox7.Text}).txt");
        ...
        {
            fwith = Path.Combine(folderbase, "C дефектом");
            fwithout = Path.Combine(folderbase, "Без дефекта");
            fdiff = Path.Combine(folderbase, "Разница");
            Directory.CreateDirectory(fwith);
            Directory.CreateDirectory(fwithout);
            Directory.CreateDirectory(fdiff);
            using (StreamWriter fs = new StreamWriter("WhereData.txt"))
                fs.WriteLine(folderbase);
        }
        ...
        {
            string[] fnames = new string[] { filenameA, filenameB, filenameC, filenameD };

            for (int i = 0; i < fnames.Length; i++)
                using (StreamWriter res = new StreamWriter(Path.Combine(fdiff, fnames[i])))
                using (StreamReader f0 = new StreamReader(Path.Combine(fwithout, fnames[i])))
                using (StreamReader f1 = new StreamReader(Path.Combine(fwith, fnames[i])))
                {
            ...
            if (checkBox2.Checked && !checkBox3.Checked)
                Furier(fdiff, folderbase);
            else
            if (checkBox3.Checked)
                ShowData(fdiff,folderbase);
        ...
        async void CalcTransform(double f0, double f1, int sc,string from,string to)
        {
            double fl = f1 - f0;
            ...
            string[] froms = new string[] {Path.Combine(from, filenameA), Path.Combine(from, filenameB), Path.Combine(from, filenameC), Path.Combine(from, filenameD) };
            string[] tosABS = new string[] { Path.Combine(to, "Abs_" + filenames[0]), Path.Combine(to, "Abs_" + filenames[1]), Path.Combine(to, "Abs_" + filenames[2]), Path.Combine(to, "Abs_" + filenames[3]) };
            string[] tos = new string[] { Path.Combine(to, filenames[0]), Path.Combine(to,  filenames[1]), Path.Combine(to,  filenames[2]), Path.Combine(to, filenames[3]) };
            ...
            toolStripStatusLabel1.Text = "Преобразование завершено. Данные записаны";
        ...
        private async void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            string dds = Path.Combine(ItFolder, filenameA);
            using (StreamWriter fs = new StreamWriter(dds))
            {
                for (int i = 0; i < n; i++)
                {
                    String x = arrA[i].ToString().Replace(',', '.');
                    fs.WriteLine(x);

                }
            }
            using (StreamWriter fs = new StreamWriter(Path.Combine(ItFolder, filenameB)))
            {
                for (int i = 0; i < n; i++)
                    fs.WriteLine(arrB[i].ToString().Replace(',', '.'));
            }
            using (StreamWriter fs = new StreamWriter(Path.Combine(ItFolder, filenameC)))
            {
                for (int i = 0; i < n; i++)
                    fs.WriteLine(arrC[i].ToString().Replace(',', '.'));
            }
            using (StreamWriter fs = new StreamWriter(Path.Combine(ItFolder, filenameD)))
            {
                for (int i = 0; i < n; i++)
        ...
        private void Furier(string from,string to = null)
        {
            if (to == null)
                to = from;
            ...
            CalcTransform(w0 / 2 / Math.PI * 1.0E6, w1 / 2 / Math.PI * 1.0E6, l,from,to);
        }

        private void ShowData(string from,string to=null)
        {
            if (to == null)
                to = from;

            string[] st = new string[] { "ArrayA", "ArrayB", "ArrayC", "ArrayD" };
            string[] names = new string[st.Length];
            for (int i = 0; i < names.Length; i++)
                names[i] = Path.Combine(from, st[i] + ".txt");

            toolStripStatusLabel1.Text = "Строится график...";
            this.Refresh();

            var form = new JustGrafic(st, names);
